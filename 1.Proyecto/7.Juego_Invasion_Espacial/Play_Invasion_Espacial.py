...
def texto_final():
    ...
    pantalla.blit(mi_fuente_final,(200,200)) #Mostramos el mensaje final en el centro
...
while se_ejecuta:
    ...
    for evento in pygame.event.get():
        ...
        if evento.type==pygame.KEYDOWN:
            ...
            if evento.key==pygame.K_RIGHT:
                jugador_x_cambio = 1
            if evento.key==pygame.K_SPACE: #Cuando se presione la tecla espaciadora
                sonido_bala=mixer.Sound('disparo.mp3')  # Cargamos el sonido de un disparo en una variable sonido_bala
                sonido_bala.play() # Ejecutar sonido de una bala cada que es disparada
                #Solo se muestra la bala cuando bala visible sea igual a falso => if not
                if not bala_visible:
                    bala_x=jugador_x
    ...
    for e in range(cantidad_enemigos):
        ...
        if colision:
            sonido_colision = mixer.Sound('Golpe.mp3')  # Cargamos el sonido de un disparo en una variable sonido_bala
            sonido_colision.play()  # Ejecutar sonido de una bala cada que es disparada
            bala_y = 500
            bala_visible = False
            # Reiniciar la posicion de los enemigos
